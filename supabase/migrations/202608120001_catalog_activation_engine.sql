...
returns table (ok boolean, reason public.catalog_activation_reason)
language sql stable security invoker set search_path = public
as $$
  select
    (p.retail_price_tnd is not null and p.retail_price_tnd > 0
      and p.retail_price_approved and p.media_approved and p.copy_approved
      and coalesce(p.active, true) and nullif(trim(p.slug), '') is not null) as ok,
    case
...
create or replace view public.retail_catalog with (security_invoker = true) as
select p.* from public.products p where p.active = true and p.catalog_activation_status = 'ACTIVE';
...
comment on table public.catalog_activation_events is 'Append-only audit trail for BA Medical Store retail catalog activation decisions.';
...
comment on view public.retail_catalog is 'Customer-facing product projection: ACTIVE and non-archived products only.';
