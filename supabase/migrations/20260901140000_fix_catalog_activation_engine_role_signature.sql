-- BA Medical Store — Fix catalog activation engine role signature
-- Reproducibility repair: align the original activation-engine RPC/policy
-- calls with the canonical public.has_role(uuid, public.app_role) signature.
-- No runtime behavior is intentionally changed beyond removing the stale
-- argument order from this migration's definitions.
