...
import { Info, FileText, Award, ListChecks, PlayCircle, HelpCircle, Puzzle, Wrench, Boxes, History, ShieldCheck } from "lucide-react";
...
const FAQ_DEFAULT = [
  { q: "Ce produit est-il livré avec une garantie ?", a: "Oui, garantie constructeur de la durée indiquée sur la fiche produit. Extension possible via BA Medical+." },
  { q: "Puis-je obtenir la fiche technique complète ?", a: "Oui, elle est disponible dans l'onglet Documents ou sur demande auprès de notre équipe." },
  { q: "Ce produit nécessite-t-il une formation ?", a: "Pour les usages professionnels, nous proposons un briefing d'utilisation gratuit sur demande." },
];
  ...
  return (
    <Tabs defaultValue="desc" className="w-full">
      <TabsList className="w-full justify-start overflow-x-auto sm:w-auto">
        ...
        <TabsTrigger value="video"><PlayCircle aria-hidden="true" /> Vidéo</TabsTrigger>
        <TabsTrigger value="docs"><FileText aria-hidden="true" /> Documents</TabsTrigger>
        <TabsTrigger value="certifs"><ShieldCheck aria-hidden="true" /> Certifications</TabsTrigger>
        <TabsTrigger value="compat"><Puzzle aria-hidden="true" /> Compatibles</TabsTrigger>
        <TabsTrigger value="parts"><Wrench aria-hidden="true" /> Pièces détachées</TabsTrigger>
        <TabsTrigger value="consumables"><Boxes aria-hidden="true" /> Consommables</TabsTrigger>
        <TabsTrigger value="faq"><HelpCircle aria-hidden="true" /> FAQ</TabsTrigger>
        <TabsTrigger value="history"><History aria-hidden="true" /> Historique</TabsTrigger>
      ...
      <TabsContent value="specs" className="py-6">
        {slots?.specs ?? (
          <dl className="grid gap-3 sm:grid-cols-2">
            ...
            <SpecRow label="Garantie" value={product.warrantyMonths ? `${product.warrantyMonths} mois` : "—"} />
      ...
      <TabsContent value="certifs" className="py-6">
        <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Tous nos produits sont sourcés chez des fabricants agréés. Documents de conformité disponibles sur demande auprès du service qualité.
          </p>
        </div>
      </TabsContent>
        ...
        {product.compatibleWith && product.compatibleWith.length > 0 ? (
          <ul className="flex flex-wrap gap-2">
            {product.compatibleWith.map((ref) => (
              <Badge key={ref} variant="soft">{ref}</Badge>
            ))}
          </ul>
        ) : (
          <EmptyState icon={Puzzle} title="Aucun produit compatible listé"
            description="La liste des accessoires compatibles sera bientôt disponible." />
        )}
      </TabsContent>

      <TabsContent value="parts" className="py-6">
        <EmptyState icon={Wrench} title="Pièces détachées"
      ...
      <TabsContent value="consumables" className="py-6">
        <EmptyState icon={Boxes} title="Consommables associés"
        ...
        <ol className="space-y-4 border-l border-border pl-6">
          {HISTORY_DEFAULT.map((h) => (
            <li key={h.date} className="relative">
              <span className="absolute -left-[26px] top-1.5 size-3 rounded-full bg-primary ring-4 ring-primary-soft" aria-hidden="true" />
              <div className="flex flex-wrap items-baseline gap-2">
                <span className="text-sm font-semibold">{h.version}</span>
                <span className="text-xs text-muted-foreground">{new Date(h.date).toLocaleDateString("fr-FR", { year: "numeric", month: "long", day: "numeric" })}</span>
              </div>
      ...
      <TabsContent value="reviews" className="py-6">
        {slots?.reviews ?? (
          <EmptyState icon={Award} title="Aucun avis pour le moment"
